...
func _ready():
	...
	if !File.new().file_exists(private_key_file):
...
func load_player_name():
	var file = File.new()
	file.open_encrypted_with_pass(highscore_file, File.READ, encryption_key)
...
func save_local_highscore(score, initial = false):
	...
	var file = File.new()
	if file.open_encrypted_with_pass(highscore_file, File.WRITE, encryption_key) != 0:
		print("Error opening file")
		return

	file.store_line(to_json({"player_name": player_name, "highscore": score}))
	file.close()

# private_key administration
func save_private_key(player_id, private_key):
	var file = File.new()
	if file.open(private_key_file, File.WRITE) != 0:
		print("Error opening file")
		return

	file.store_line(to_json({"player_id": player_id, "private_key": private_key}))
	file.close()


func load_private_key():
	var file = File.new()
	file.open(private_key_file, File.READ)
	var data = parse_json(file.get_as_text())
...
func make_local_highscore_file():
	...
	file.store_line(to_json({"player_name": player_name, "highscore": null}))
	file.close()
...
func submit_highscore(highscore):
	http_request.cancel_request()
	var encrypted_highscore = aes.pool_byte_array_to_byte_string(aes.encrypt(str(highscore), encryption_key))
	...
	if private_key && player_id:
		...
		url = str(host + "/add_player_and_submit_entry.php?public_key=" + str(public_key) + "&name=" + player_name.http_escape() + "&score=" + str(encrypted_highscore))
...
class AES:
	# this code is written by beamer159
	...
	# added by kitchen-games(Merten Koch-Menzel 2021

	#returns the given array as a string splitted by "_"
